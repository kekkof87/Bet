...
if page == "Alerts":
    ...
    data = fetch("/fixtures", fallback_file="last_delta.json", params=params)
    items = data.get("items", data.get("added", []))
    df = pd.DataFrame(items)
    st.dataframe(df, use_container_width=True)
    ...
    df_hist = pd.DataFrame(hist.get("items", []))
    if not df_hist.empty:
        # Chart semplice se presenti colonne standard: date, roi
        for date_col in ["date", "day", "timestamp"]:
            if date_col in df_hist.columns:
                df_hist[date_col] = pd.to_datetime(df_hist[date_col], errors="coerce")
        num_cols = [c for c in df_hist.columns if c.lower() in ["roi", "cum_roi", "yield", "bankroll"] or c.endswith("roi")]
        if num_cols:
            date_col = next((c for c in ["date","day","timestamp"] if c in df_hist.columns), None)
            if date_col:
                chart = (
