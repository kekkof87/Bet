...
def test_load_missing_returns_empty(monkeypatch, tmp_path) -> None:
...
def test_save_skips_empty(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("BET_DATA_DIR", str(tmp_path))
    save_latest_fixtures([])
    assert not LATEST_FIXTURES_FILE.exists()


def test_invalid_json_returns_empty_and_warn(monkeypatch, tmp_path, caplog) -> None:
    monkeypatch.setenv("BET_DATA_DIR", str(tmp_path))
    ...
    assert out == []
    warnings = [
        r for r in caplog.messages if "invalid" in r.lower() or "corrupt" in r.lower()
    ]
    assert warnings


def test_non_list_structure_returns_empty_and_warn(
    monkeypatch,
    tmp_path,
    caplog,
) -> None:
