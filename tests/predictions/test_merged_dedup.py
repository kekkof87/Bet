...
from predictions.value_alerts import build_value_alerts
...
def _write_pred_cons(tmp_path: Path):
    ...
    (pdir / "latest_predictions.json").write_text(json.dumps({
        "predictions": [
            {
                "fixture_id": 999,
                "model_version": "v1",
                "value": {"active": True, "value_side": "home_win", "value_edge": 0.07}
            }
        ]
    }), encoding="utf-8")
    ...
    (cdir / "consensus.json").write_text(json.dumps({
        "entries": [
            {
                "fixture_id": 999,
                "consensus_value": {"active": True, "value_side": "home_win", "value_edge": 0.10}
            }
        ]
    }), encoding="utf-8")
...
def test_dedup(tmp_path: Path):
    ...
    # Con dedup merged abilitato prevediamo SOLO merged (no prediction+consensus)
